...
void Solve() {
    int n; cin >> n;
    vll data(2*n), diff1 = {0}, diff2 = {0};
    FOR(i, 0, 2*n) {
        cin >> data[i];
        if (i & 1) {
            diff1.pb(diff1.back() + data[i] - data[i-1]);
        } else {
            if (i > 0) diff2.pb(diff2.back() + data[i] - data[i-1]);
        }
    }

    int k = 0, l = n, a = 0, b = n-1;
    ll fix = 0;
    FOR(i, 0, n) {
        if (i & 1) {
            cout << fix + diff2[b--] - diff2[a++] << ' ';
        } else {
            cout << fix + diff1[l--] - diff1[k++] << ' ';
        }
        fix += data[2*n - i - 1] - data[i];
    }
    cout << '\n';
}
