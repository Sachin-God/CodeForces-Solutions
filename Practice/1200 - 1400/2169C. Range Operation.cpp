...
// The below Solution is wrong as mn might occur after mx
// this is not enForcing order
void Wrong() {
    int n; cin >> n;
    vll a(n+1, 0), fx(n+1, 0);
    FOR(i, 0, n) cin >> a[i+1];
    ll mn = 0, mx = LLONG_MIN, ps = 0;
    FOR(i, 1, n+1) {
        ps += a[i];
        fx[i] = 1LL * i*(i+1) - ps;
        mx = max(mx, fx[i]);
...
void Solve() {
    ...
    vll a(n+1, 0);
    FOR(i, 0, n) cin >> a[i+1];
    ll mn = 0, maxDiff = 0, ps = 0, curr;
    FOR(i, 1, n+1) {
        ps += a[i];
        curr = 1LL * i*(i+1) - ps;
        mn = min(mn, curr);
        maxDiff = max(maxDiff, curr-mn);
    }
