...
void Solve() {
    ...
    FOR(i, 1, n) {
        without.pb(max(without[i-1]+a[i], a[i]));
        with.pb(max(max(without[i-1]+a[i]+b[i], a[i]+b[i]), with[i-1]+a[i]));
    }

    ll a1 = *max_element(all(with)), a2 = *max_element(all(without));;
    if (k&1) cout << max(a1, a2);
    else cout << a2;
    cout << "\n";
}
